...
{
   int slen, i, m_b, n_b, te = -1, gmax = 0, minsc, endsc,segNum;
   uint64_t *b;
   __m256i ninf, oe_del, e_del, oe_ins, e_ins, *H0, *H1, *E, *Hmax;
   kswr_t r;

   const int32_t segLen = (q->slen + 16 - 1) / 16;
   const int32_t position = (16 - 1) - (q->slen - 1) / segLen;	
...
#define __max_8(ret, xx) do { \
   ...
   Hmax = q->Hmax;
   slen = q->slen;
   int16_t* const restrict boundary =(int16_t*)malloc(sizeof(int16_t)*tlen);
    int32_t index = 0;
        for (i=0; i<segLen; ++i) {
            __m256i_16_t h;
            __m256i_16_t e;
            for (segNum=0; segNum<16; ++segNum) {
                int64_t tmp = -(_o_del + _e_del)*(segNum*segLen+i);
                h.v[segNum] = tmp < INT16_MIN ? INT16_MIN : tmp;
                tmp = tmp - _o_del;
                e.v[segNum] = tmp < INT16_MIN ? INT16_MIN : tmp;
            }
            _mm256_store_si256(&H0[index], h.m);
            _mm256_store_si256(&E[index], e.m);
            ++index;
        }
    printf("OK");
   ...
   for (i = 0; i < tlen; ++i) {
      int j, k, imax;       
      __m256i mask,e, t, h, f = ninf, max = ninf, *S = q->qp + target[i] * slen; // s is the 1st score vector
      h = _mm256_load_si256(H0 + slen -1); // h={2,5,8,11,14,17,-1,-1} in the above example
	...
	h = _mm256_insert_epi16_rpl(h, boundary[i], 0);
      for (j = 0; LIKELY(j < slen); ++j) {
         h = _mm256_add_epi16(h, _mm256_load_si256(S + j));
        ...
        // max = _mm256_max_epi16(max, h);
         ...
         e = _mm256_sub_epi16(e, e_del);
         h = _mm256_sub_epi16(h, oe_del);
         e = _mm256_max_epi16(e, h);
         _mm256_store_si256(E + j, e);
         f = _mm256_sub_epi16(f, e_ins);
       //  t = _mm256_sub_epi16(h, oe_ins);
	 ...
	 mask = _mm256_permute2x128_si256(f, f, _MM_SHUFFLE(0,0,3,0) );
      ...
      H0 = S;
    {
        __m256i h = _mm256_load_si256(H0 + (slen - 1) % segLen);
        for (k=0; k<position; ++k) {
            h = _mm256_slli_si256_rpl (h, 2);
        }
        r.score = (int16_t) _mm256_extract_epi16_rpl (h, 15);
    }
	return r;
}
